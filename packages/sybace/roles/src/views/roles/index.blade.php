    ...
    <div class="row">
        <div class="col-md-12">
            <div class="admin-top-operation">
                <a class="btn btn-default" href="{{ action('\Sybace\Roles\Controllers\RolesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Roles::roles.role') }}</a>
            </div>
            @if($items->count() || $_GET)
                @include('Roles::roles.filter')
            @endif
            @if($items->count())
                <form method="POST" action="{{ action('\Sybace\Roles\Controllers\RolesController@bulkOperations')}}" id="bulk" class="form-inline">
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            @foreach($items as $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    @endif
                                    <td>
                                        <a href="{{ action('\Sybace\Roles\Controllers\RolesController@edit', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
            ...
            @else
                <div class="text-center">
                    @if($_GET)
                        <h1>{{ trans('admin.no_results_found') }} <a href="{{ action('\Sybace\Roles\Controllers\RolesController@index')}}">{{ trans('admin.back') }}</a></h1>
                    @elseif(Request::is(''.Lang::getlocale().'/admin/roles'))
                        <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\Sybace\Roles\Controllers\RolesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Roles::roles.role') }}</a></h1>
                    @endif
                </div>
            @endif
        </div>
