...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="admin-top-operation">
            <a class="btn btn-default" href="{{ action('\Sybace\Articles\Controllers\ArticlesController@create')}}">{{ trans('Articles::articles.create') }} {{ trans('Articles::articles.article') }}</a>
        </div>

        @if($items->count())
            <form method="POST" action="{{ action('\Sybace\Articles\Controllers\ArticlesController@bulkOperations')}}" id="bulk" class="form-inline">
                            ...
                            @foreach($items as $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    <td>
                                        ...
                                        @endif
                                    <td>
                                       <a href="{{ action('\Sybace\Articles\Controllers\ArticlesController@edit', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
        ...
        @else
            <div class="text-center">
                @if($_GET)
                    <h1>{{ trans('admin.no_results_found') }} <a href="{{ action('\Sybace\Articles\Controllers\ArticlesController@index')}}">{{ trans('admin.back') }}</a></h1>
                @elseif(Request::is(''.Lang::getlocale().'/admin/ads'))
                    <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\Sybace\Articles\Controllers\ArticlesController@create')}}">{{ trans('Ads::ads.create') }} {{ trans('Ads::ads.ad') }}</a></h1>
                @endif
             </div>
        @endif
    </div>
