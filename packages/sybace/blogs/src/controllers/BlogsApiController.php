   ...
   */
    ...
    public function storeBlog(Request $request)
    {
        ...
        foreach ($request->language as $langCode) {
            ...
            $blogs_trans = new BlogTrans;
            $blogs_trans->post_id = $blogs->id;
            $blogs_trans->name = ucwords(strtolower($request->$name));
            $blogs_trans->body = $request->$description;
            $blogs_trans->lang = $langCode;
            $blogs_trans->save();
        }
...
// Get SEO URL function here
    ...
    public function updateBlog(Request $request, $apiKey = '', $id) {
        ...
        foreach ($request->language as $langCode) {
            $name = 'name_' . $langCode;
            $blogTrans = BlogTrans::where('blog_id', $blog->id)->where('lang', $langCode)->first();
            if (empty($blogTrans)) {
            ...
            $blogTrans->body = $request->description;
            $blogTrans->save();
        }
