    ...
    <div class="row">
        <div class="col-md-12">
            <div class="admin-top-operation">
                <a class="btn btn-default" href="{{ action('\Sybace\Blogs\Controllers\BlogsController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Blogs::blogs.blog') }}</a>
            </div>
            @if($items->count() || $_GET)
            ...
            @else
                <div class="text-center">
                    <h1>{{ trans('admin.no_results_found') }}
                        {{--<a href="{{ action('\Sybace\Blogs\Controllers\BlogsController@index')}}">{{ trans('admin.back') }}</a>--}}
                    </h1>
                </div>
            @endif
        </div>
