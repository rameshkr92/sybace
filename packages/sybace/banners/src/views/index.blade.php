...
<div class="row">
    <div class="col-md-12">
        <div class="admin-top-operation">
            <a class="btn btn-default" href="{{ action('\Sybace\Banners\Controllers\BannersController@create')}}">{{ trans('Banners::banners.create') }} {{ trans('Banners::banners.banner') }}</a>
        </div>
         @if($items->count() || $_GET)
            @include('Banners::filter')
        @endif
        @if($items->count())
        <form method="POST" action="{{ action('\Sybace\Banners\Controllers\BannersController@bulkOperations')}}" id="bulk" class="form-inline">
                    ...
                    {{--{{ dd($items) }}--}}
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @endif
                            <td>
                                <a href="{{ action('\Sybace\Banners\Controllers\BannersController@edit', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
        ...
        <div class="text-center">
            @if($_GET)
            <h1>{{ trans('admin.no_results_found') }} <a href="{{ action('\Sybace\Banners\Controllers\BannersController@index')}}">{{ trans('admin.back') }}</a></h1>
            @elseif(Request::is(''.Lang::getlocale().'/admin/ads'))
            <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\Sybace\Banners\Controllers\BannersController@create')}}">{{ trans('Ads::ads.create') }} {{ trans('Ads::ads.ad') }}</a></h1>
            @endif
        </div>
        @endif
    </div>
