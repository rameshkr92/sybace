...
if(!empty(\Request::segment(3)) && \Request::segment(3) == "contactus" && intval(\Request::segment(4)) != 0) {
    $contact_request = \Sybace\Contactus\Models\ContactusTrans::where('contact_id', \Request::segment(4))->first();
    if (count($contact_request) > 0){
        $contact_request->is_read = "1";
        $contact_request->save();
    }
}
//$unread_contact = \Sybace\Contactus\Models\ContactusTrans::where('is_read', "0")->count();
$unread_contact = 0;
$adminMenu[12] = [
    'icon' => 'table',
    'list_head' => [
