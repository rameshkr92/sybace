    ...
    <div class="row">
        <div class="col-md-12">
            <div class="admin-top-operation">
                <a class="btn btn-default" href="{{ action('\Sybace\Contactus\Controllers\ContactusController@createSection')}}">{{ trans('Core::operations.create') }} {{ trans('Contactus::contactus.contactus_section') }}</a>
            </div>
            @if($items->count() || $_GET)
{{--                @include('Contactus::contactus.filter')--}}
            @endif
            @if($items->count())
                <form method="POST" action="{{ action('\Sybace\Contactus\Controllers\ContactusController@bulkOperationsSection')}}" id="bulk" class="form-inline">
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            @foreach($items as $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    <td>
                                        <a href="{{ action('\Sybace\Contactus\Controllers\ContactusController@editSection', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
            ...
            @else
                <div class="text-center">
                    @if($_GET)
                        <h1>{{ trans('admin.no_results_found') }} <a href="{{ action('\Sybace\Activities\Controllers\ActivitiesController@index')}}">{{ trans('admin.back') }}</a></h1>
                    @elseif(Request::is(''.Lang::getlocale().'/admin/activities'))
                        <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\Sybace\Activities\Controllers\ActivitiesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Contactus::contactus.activity') }}</a></h1>
                    @endif
                </div>
            @endif
        </div>
