...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="admin-top-operation">
            <a class="btn btn-default" href="{{ action('\Sybace\Countries\Controllers\CountriesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Countries::countries.country') }}</a>
        </div>
        ...
        @if($items->count())
            <form method="POST" action="{{ action('\Sybace\Countries\Controllers\CountriesController@bulkOperations')}}" id="bulk" class="form-inline">
                            ...
                            @foreach($items as $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    <td>
                                        ...
                                        @endif
                                    <td>
                                       <a href="{{ action('\Sybace\Countries\Controllers\CountriesController@edit', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
        ...
        @else
            <div class="text-center">
                @if($_GET)
                    <h1>{{ trans('admin.no_results_found') }} <a href="{{ action('\Sybace\Countries\Controllers\CountriesController@index')}}">{{ trans('admin.back') }}</a></h1>
                @elseif(Request::is(''.Lang::getlocale().'/admin/countries'))
                    <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\Sybace\Countries\Controllers\CountriesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Countries::countries.country') }}</a></h1>
                @endif
             </div>
        @endif
    </div>
