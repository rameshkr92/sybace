...
use Sybace\Users\Models\Account;
use Sybace\Countries\Models\Country;
use Sybace\Countries\Models\Area;
use Sybace\Countries\Models\Province;
use Sybace\Countries\Models\City;
use Sybace\Countries\Models\District;
    ...
    public function register(Request $request)
    {
        ...
        try {
            Mail::send('emails.active_user', ['user' => $user], function ($msg) use ($user) {
                $msg->from('matrash@sybace.com', 'Matrash');
                $msg->to($user->email);
        ...
        if(is_numeric($username)){
            ...
            try {
                Mail::send('emails.reset_password', ['user' => $user], function ($msg) use ($user) {
                    $msg->from('mala3eb@sybace.com', 'Mala3eb');
                    $msg->to($user->email);
