    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="admin-top-operation">
                <a class="btn btn-default" href="{{ action('\Sybace\Users\Controllers\UsersController@create', $role)}}">{{ trans('Core::operations.create') }} {{ $itemTitle }}</a>
            </div>
            ...
            @if($items->count())
                <form method="POST" action="{{ action('\Sybace\Users\Controllers\UsersController@bulkOperations')}}" id="bulk" class="form-inline">
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            @foreach($items as $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    {{--@if(Request::segment(4)== 'supervisors')--}}
                                        {{--<td>--}}
                                            {{--<a class="btn btn-sm btn-danger" href="{{ action('\Sybace\Users\Controllers\UsersController@givePermission', [$role, $item->id]) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.permission') }}</a>--}}
                                        {{--</td>--}}
                                    {{--@endif--}}
                                    <td>
                                        <a href="{{ action('\Sybace\Users\Controllers\UsersController@edit', [$role, $item->id]) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
            ...
            @else
                <div class="text-center">
                    @if($_GET)
                        <h1>{{ trans('admin.no_results_found') }} <a href="{{ action('\Sybace\Users\Controllers\UsersController@index', $role)}}">{{ trans('admin.back') }}</a></h1>
                    @else
                        <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\Sybace\Users\Controllers\UsersController@create', $role)}}">{{ trans('Users::users.create') }} {{ $itemTitle }}</a></h1>
                    @endif
                </div>
            @endif
        </div>
