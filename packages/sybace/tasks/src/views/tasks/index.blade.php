    ...
    <div class="row">
        <div class="col-md-12">
            <div class="admin-top-operation">
                <a class="btn btn-default" href="{{ action('\Sybace\Stores\Controllers\StoresController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Stores::stores.store') }}</a>
            </div>
            @if($items->count() || $_GET)
                @include('Stores::stores.filter')
            @endif
            @if($items->count())
                <form method="POST" action="{{ action('\Sybace\Stores\Controllers\StoresController@bulkOperations')}}" id="bulk" class="form-inline">
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            @foreach($items as $key => $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    @endif
                                    <td>
                                        <a href="{{ action('\Sybace\Stores\Controllers\StoresController@edit', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
            ...
            @else
                <div class="text-center">
                    @if($_GET)
                        <h1>{{ trans('admin.no_results_found') }} <a href="{{ action('\Sybace\Stores\Controllers\StoresController@index')}}">{{ trans('admin.back') }}</a></h1>
                    @elseif(Request::is(''.Lang::getlocale().'/admin/stores'))
                        <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\Sybace\Stores\Controllers\StoresController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Stores::stores.store') }}</a></h1>
                    @endif
                </div>
            @endif
        </div>
