@php
    ...
    $action = action('\Sybace\Stores\Controllers\StoresController@store');
    $method = '';
    ...
    if(Request::is('*/edit')){
        ...
        $action = action('\Sybace\Stores\Controllers\StoresController@update', $item->id);
        $method = 'PATCH';
...
@section('footer')
    ...
    <script type="text/javascript">
        function deleteMyImages(id)
        {
            ...
            {
                $.ajax({
                    url: '{{ action('\Sybace\Stores\Controllers\StoresController@deleteStoreGalleryImages') }}', //The url where the server req would we made.
