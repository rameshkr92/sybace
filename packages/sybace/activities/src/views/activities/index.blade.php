    ...
    <div class="row">
        <div class="col-md-12">
            <div class="admin-top-operation">
                <a class="btn btn-default" href="{{ action('\Sybace\Activities\Controllers\ActivitiesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Activities::activities.activity') }}</a>
            </div>
            @if($items->count() || $_GET)
                @include('Activities::activities.filter')
            @endif
            @if($items->count())
                <form method="POST" action="{{ action('\Sybace\Activities\Controllers\ActivitiesController@bulkOperations')}}" id="bulk" class="form-inline">
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            @foreach($items as $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    @endif
                                    <td>
                                        <a href="{{ action('\Sybace\Activities\Controllers\ActivitiesController@edit', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
            ...
            @else
                <div class="text-center">
                    ...
                    @elseif(Request::is(''.Lang::getlocale().'/admin/activities'))
                        <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\Sybace\Activities\Controllers\ActivitiesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Activities::activities.activity') }}</a></h1>
                    @endif
                </div>
            @endif
        </div>
